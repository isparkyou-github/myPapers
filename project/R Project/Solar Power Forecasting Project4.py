# -*- coding: utf-8 -*-
"""Solar Power Forecasting Project4.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DB2n88z7uyq-GddmizJzycQeYMiuolPv
"""
...
cat("Parsing datetime information...\n")
combined_dataset$date_time <- as.POSIXct(combined_dataset$date_time, format="%Y-%m-%d %H:%M:%S")  # 解析时间数据
cat("First few rows of the parsed time column:\n")
print(head(combined_dataset$date_time))

# hour, minute, second
combined_dataset$hour <- as.numeric(format(combined_dataset$date_time, "%H"))
combined_dataset$minute <- as.numeric(format(combined_dataset$date_time, "%M"))
combined_dataset$second <- as.numeric(format(combined_dataset$date_time, "%S"))

# Visualize time data
ggplot(combined_dataset, aes(x = date_time, y = power)) +
  geom_line(color = "blue") +
  labs(title = "Power over Time", x = "Time", y = "Power") +
  theme_minimal()
